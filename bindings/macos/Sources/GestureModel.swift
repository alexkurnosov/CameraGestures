...
public class GestureModel {
    ...
    /// Return ALL gesture predictions sorted by confidence descending, with no threshold.
    /// Includes every class the model knows (excluding _none — see the noneConfidence
    /// helper).  Never throws; returns [] if the model is not loaded or the film is empty.
    public func predictAllGestures(handfilm: HandFilm) -> [GesturePrediction] {
        guard let ref = modelRef, !handfilm.frames.isEmpty else { return [] }
        let film = handfilm.toCRef()
        defer { cg_handfilm_destroy(film) }
        let cap = 30  // more than enough for any gesture registry
        var buf = [cg_gesture_prediction](repeating: cg_gesture_prediction(), count: cap)
        let n = cg_gesture_model_classify_topk(ref, film, Int32(cap), 0.0, &buf, Int32(cap))
